...
export class ApiDataService {
  ...
  loginUser(): Observable<LoginModel> {
    return this.http.post<LoginModel>(`http://8531-106-201-236-89.ngrok.io/api/LogIn`, this.user, { responseType: 'json' })
  }
  ...
  getToken(email: string, password: string): Observable<LoginModel> {
    ...
    return this.http.post<LoginModel>(`http://8531-106-201-236-89.ngrok.io/Token`, body, { headers: header });
  }

  addSupplier(): Observable<Supplier> {
    console.log(this.supplier);
    return this.http.post<Supplier>(`http://8531-106-201-236-89.ngrok.io/api/Suppliers`, this.supplier);
  }

  editSupplier(id: number, token: string): Observable<Supplier> {
    let supplierHeader = new HttpHeaders().set('Authorization', 'bearer ' + token)
    return this.http.put<Supplier>(`http://8531-106-201-236-89.ngrok.io/api/Suppliers/` + id, this.supplier, { headers: supplierHeader });
  }

  getDataSuppliers() {
    return this.http.get(`http://8531-106-201-236-89.ngrok.io/api/Suppliers`);
  }

  deleteSupplier(supplierId: number, token: string) {
    let deleteSupplierHeader = new HttpHeaders().set('Authorization', 'bearer ' + token);
    return this.http.delete(`http://8531-106-201-236-89.ngrok.io/api/Suppliers/` + supplierId, { headers: deleteSupplierHeader });
  }

  updateSupplierStatus(id: number, status: boolean) {
    let url = `http://8531-106-201-236-89.ngrok.io/api/Suppliers/?id=` + id + `&isActive=` + status;
