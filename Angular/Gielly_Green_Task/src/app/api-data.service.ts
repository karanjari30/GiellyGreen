...
export class ApiDataService {
  ...
  baseAPIUrl = `http://apiteam1.azurewebsites.net/`;
  token: any;
  user: LoginModel =
    {
  ...
  loginUser(): Observable<LoginModel> {
    return this.http.post<LoginModel>(`${this.baseAPIUrl}api/LogIn`, this.user, { responseType: 'json' })
  }
  ...
  updateSupplierStatus(id: number, status: boolean, token: string) {
    ...
    let url = `${this.baseAPIUrl}api/Suppliers/?id=` + id + `&isActive=` + status;
  ...
  getMonthlyInvoiceData(month: number, year: number, token: string) {
    ...
    return this.http.get(`${this.baseAPIUrl}api/MonthlyInvoices?month=` + month + `&year=` + year, { headers: getMonthlyInvoiceDataHeader })
  }
  ...
  addMonthlyInvoicesData(monthlyJsonObject: any, token: string): Observable<Monthlyinvoice> {
    ...
    return this.http.post<Monthlyinvoice>(`${this.baseAPIUrl}api/MonthlyInvoices`, monthlyJsonObject, { headers: addmonthlyInvoiceDataHeader });
  }
  ...
  approveSupplier(idArray: any, month: number, year: number, token: string) {
    ...
    let url = `${this.baseAPIUrl}api/MonthlyInvoices?month=` + month + `&year=` + year;
  ...
  emailSelectedSuppliers(arrayOfID: number, month: number, year: number, token: string) {
    ...
    let url = `${this.baseAPIUrl}api/Email?month=` + month + `&year=` + year;
  ...
  downloadPDF(IDArray: number, month: number, year: number, token: string) {
    ...
    let url = `${this.baseAPIUrl}api/PDF?month=` + month + `&year=` + year;
    return this.http.post(url, IDArray, { headers: downloadPDFHeader });
  }

  getProfileData(){
    return this.http.get(`${this.baseAPIUrl}api/Profile`);
  }

  setProfileData(jsonObject:any): Observable<Profile>{
    let id = jsonObject.CId;
    return this.http.post<Profile>(`${this.baseAPIUrl}api/Profile/`+ id, jsonObject);
  }
